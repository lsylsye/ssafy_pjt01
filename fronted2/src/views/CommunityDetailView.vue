...
import { getPostDetail, getComments, toggleLike, createComment, deleteComment, toggleCommentLike } from '@/api/community';
...
const handleCommentLike = async (commentId) => {
    ...
    try {
        ...
        // 댓글 목록만 갱신 (또는 해당 댓글만 찾는 로직도 가능하지만 지금은 전체 갱신이 안전)
...
const handleLike = async () => {
    ...
    try {
        ...
        const res = await getPostDetail(post.value.id);
        post.value = res.data; 
    } catch (e) {
        console.error(e);
                ...
                <div class="comment-item">
                    ...
                    <div class="comment-bubble">
                        ...
                        <div class="comment-actions">
                            <button 
                                class="action-btn" 
                                :class="{ active: cmt.is_liked }"
                                @click="handleCommentLike(cmt.id)"
                            >
                                <Heart :size="14" :fill="cmt.is_liked ? 'currentColor' : 'none'" />
                                좋아요 {{ cmt.like_count }}
                            </button>
