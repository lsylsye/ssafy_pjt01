...
def _build_profile_response(request, user):
    ...
    return {
        ...
        "favorite_country_display": user.other_country if user.favorite_country == "OTHER" else (user.get_favorite_country_display() if user.favorite_country else ""),
        "other_country": getattr(user, "other_country", ""),
        "favorite_genre": getattr(user, "favorite_genre", None),
        "favorite_genre_display": user.get_favorite_genre_display() if user.favorite_genre else "",
...
def my_posts(request):
    ...
    for p in posts:
        items.append({
            ...
            "type_name": "자유",
            "title": p.title,
            "prefix_name": p.prefix.name if p.prefix else "",
